...
export default function WithdrawHistory() {
    ...
    const fetchDepositHistory = async () => {
        try {
            setLoading(true);
            const response = await fetch(`${BaseUrl}/withdraw-history/${id}`);
    ...
    return (
        <div className="flex flex-col items-center h-screen w-full">
            <div className="flex flex-row items-center w-[90%] sticky top-0 py-2 bg-white">
                ...
                <span className="text-[16px] font-semibold ml-2 w-[80%] line-clamp-1">Withdraw Record</span>
            </div>
            <div className="w-full flex flex-col items-center">
                <div className="flex flex-col items-center w-[90%] mt-[5%]">
                    {loading ? (
                        <LoadingSpinner />
                    ) : depositHistory.length === 0 ? (
                        <p className="text-xl font-semibold text-[#347928] mt-[50%]">No Withdraw found.</p>
                    ) : (
                        depositHistory?.map((item, index) => (
                            <div key={index} className="flex flex-col items-center w-full p-2 py-4 border-b last:border-b-0 border shadow-sm rounded-md mt-2">
